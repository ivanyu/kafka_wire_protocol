...
pub(crate) fn read_bytes(input: &mut impl Read, field_name: &str, compact: bool) -> Result<Vec<u8>> {
    ...
    if len < 0 {
        ...
        read_bytes_inner(input, len)
    }
}

pub(crate) fn read_nullable_bytes(input: &mut impl Read, field_name: &str, compact: bool) -> Result<Option<Vec<u8>>> {
    let len = read_len_i32(input, invalid_len_message(field_name), compact)?;
    if len < 0 {
        Ok(None)
    } else {
        read_bytes_inner(input, len).map(Some)
    }
}

#[inline]
fn read_bytes_inner(input: &mut impl Read, str_len: i32) -> Result<Vec<u8>> {
    let mut buf = vec![0_u8; str_len as usize];
    input.read_exact(&mut buf)?;
    Ok(buf)
}

pub(crate) fn write_bytes(output: &mut impl Write, field_name: &str, value: &[u8], compact: bool) -> Result<()> {
    let len = value.len();
...
pub(crate) fn write_nullable_bytes(output: &mut impl Write, field_name: &str, value: Option<&[u8]>, compact: bool) -> Result<()> {
    if let Some(v) = value {
        write_bytes(output, field_name, v, compact)
    } else {
...
mod tests {
    ...
    fn check_serde_nullable(original_data: Option<Vec<u8>>, compact: bool) {
        ...
        write_nullable_bytes(&mut cur, "test", original_data.as_deref(), compact).unwrap();

        cur.seek(SeekFrom::Start(0)).unwrap();
        let read_data = read_nullable_bytes(&mut cur, "test", compact).unwrap();
    ...
    fn check_serde_non_nullable(original_data: Vec<u8>, compact: bool) {
        let mut cur = Cursor::new(Vec::<u8>::new());
        write_bytes(&mut cur, "test", &original_data, compact).unwrap();

        cur.seek(SeekFrom::Start(0)).unwrap();
        let read_data = read_bytes(&mut cur, "test", compact).unwrap();
    ...
    fn test_read_null_bytes_non_nullable_non_compact() {
        ...
        cur.seek(SeekFrom::Start(0)).unwrap();
        let error = read_bytes(&mut cur, "test", false)
    ...
    fn test_read_null_bytes_non_nullable_compact() {
        ...
        cur.seek(SeekFrom::Start(0)).unwrap();
        let error = read_bytes(&mut cur, "test", true)
            .expect_err("must be error");
        assert_eq!(error.to_string(), "non-nullable field test was serialized as null");
    }
    ...
    fn test_read_long_bytes_nullable_compact() {
        ...
        cur.write_u32_varint(i32::MAX as u32 + 2).unwrap();
        cur.seek(SeekFrom::Start(0)).unwrap();
        let error = read_nullable_bytes(&mut cur, "test", true)
            .expect_err("must be error");
        assert_eq!(error.to_string(), "bytes field test had invalid length 2147483648");
    }
}
