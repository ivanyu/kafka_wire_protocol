...
/// DeleteAclsFilterResult, version 0.
#[derive(Serialize, Deserialize, PartialEq, Debug, Clone)]
#[cfg_attr(test, derive(Arbitrary))]
...
/// DeleteAclsMatchingAcl, version 0.
#[derive(Serialize, Deserialize, PartialEq, Debug, Clone)]
#[cfg_attr(test, derive(Arbitrary))]
...
impl DeleteAclsMatchingAcl {
    pub fn new<S1: AsRef<str>, S2: AsRef<str>, S3: AsRef<str>, S4: AsRef<str>>(error_code: i16, error_message: Option<S1>, resource_type: i8, resource_name: S2, principal: S3, host: S4, operation: i8, permission_type: i8) -> Self {
        Self {
            error_code,
            error_message: error_message.map(|s| s.as_ref().to_string()),
            resource_type,
...
impl Readable for DeleteAclsMatchingAcl {
    fn read(#[allow(unused)] input: &mut impl Read) -> Result<Self> {
        ...
        Ok(DeleteAclsMatchingAcl {
            error_code, error_message, resource_type, resource_name, principal, host, operation, permission_type
        })
    }
}
...
mod tests {
    ...
    fn test_java_default() {
        crate::test_utils::test_java_default::<DeleteAclsResponse>("DeleteAclsResponse", 0);
    }
    ...
    proptest! {
        ...
        fn test_java_arbitrary(data: DeleteAclsResponse) {
            crate::test_utils::test_java_arbitrary(&data, "DeleteAclsResponse", 0);
        }
    }
}
