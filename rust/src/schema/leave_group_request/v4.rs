...
impl Readable for MemberIdentity {
    fn read(#[allow(unused)] input: &mut impl Read) -> Result<Self> {
        ...
        let _unknown_tagged_fields = read_tagged_fields(input, tagged_fields_callback)?;
        Ok(MemberIdentity {
            member_id, group_instance_id, _unknown_tagged_fields
        })
    }
}
...
mod tests {
    ...
    fn test_java_default() {
        crate::test_utils::test_java_default::<LeaveGroupRequest>("LeaveGroupRequest", 4);
    }
    ...
    proptest! {
        ...
        fn test_java_arbitrary(data: LeaveGroupRequest) {
            crate::test_utils::test_java_arbitrary(&data, "LeaveGroupRequest", 4);
        }
    }
}
