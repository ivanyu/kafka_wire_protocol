...
/// LeaderAndIsrTopicState, version 3.
#[derive(Serialize, Deserialize, PartialEq, Debug, Clone)]
#[cfg_attr(test, derive(Arbitrary))]
...
/// LeaderAndIsrLiveLeader, version 3.
#[derive(Serialize, Deserialize, PartialEq, Debug, Clone)]
#[cfg_attr(test, derive(Arbitrary))]
...
/// LeaderAndIsrPartitionState, version 3.
#[derive(Serialize, Deserialize, PartialEq, Debug, Clone)]
#[cfg_attr(test, derive(Arbitrary))]
...
impl LeaderAndIsrPartitionState {
    pub fn new(partition_index: i32, controller_epoch: i32, leader: i32, leader_epoch: i32, isr: Vec<i32>, partition_epoch: i32, replicas: Vec<i32>, adding_replicas: Vec<i32>, removing_replicas: Vec<i32>, is_new: bool) -> Self {
        Self {
            partition_index,
            controller_epoch,
            leader,
...
impl Readable for LeaderAndIsrPartitionState {
    fn read(#[allow(unused)] input: &mut impl Read) -> Result<Self> {
        ...
        let adding_replicas = read_array::<i32>(input, "adding_replicas", false)?;
        let removing_replicas = read_array::<i32>(input, "removing_replicas", false)?;
        let is_new = bool::read(input)?;
        Ok(LeaderAndIsrPartitionState {
            partition_index, controller_epoch, leader, leader_epoch, isr, partition_epoch, replicas, adding_replicas, removing_replicas, is_new
        })
    }
}
...
impl Writable for LeaderAndIsrPartitionState {
    fn write(&self, #[allow(unused)] output: &mut impl Write) -> Result<()> {
        ...
        write_array(output, "self.adding_replicas", &self.adding_replicas, false)?;
        write_array(output, "self.removing_replicas", &self.removing_replicas, false)?;
...
mod tests {
    ...
    fn test_java_default() {
        crate::test_utils::test_java_default::<LeaderAndIsrRequest>("LeaderAndIsrRequest", 3);
    }
    ...
    proptest! {
        ...
        fn test_java_arbitrary(data: LeaderAndIsrRequest) {
            crate::test_utils::test_java_arbitrary(&data, "LeaderAndIsrRequest", 3);
        }
    }
}
