...
impl Readable for Voter {
    fn read(#[allow(unused)] input: &mut impl Read) -> Result<Self> {
        ...
        let _unknown_tagged_fields = read_tagged_fields(input, tagged_fields_callback)?;
        Ok(Voter {
            voter_id, _unknown_tagged_fields
        })
    }
}
...
mod tests {
    ...
    fn test_java_default() {
        crate::test_utils::test_java_default::<LeaderChangeMessage>("LeaderChangeMessage", 0);
    }
    ...
    proptest! {
        ...
        fn test_java_arbitrary(data: LeaderChangeMessage) {
            crate::test_utils::test_java_arbitrary(&data, "LeaderChangeMessage", 0);
        }
    }
}
