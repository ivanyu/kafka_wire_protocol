...
impl Readable for WriteTxnMarkersRequest {
    fn read(#[allow(unused)] input: &mut impl Read) -> Result<Self> {
        let markers = read_array::<WritableTxnMarker>(input, "markers", false)?;
...
impl Readable for WritableTxnMarker {
    fn read(#[allow(unused)] input: &mut impl Read) -> Result<Self> {
        ...
        let topics = read_array::<WritableTxnMarkerTopic>(input, "topics", false)?;
...
mod tests {
    ...
    fn test_java_default() {
        crate::test_utils::test_java_default::<WriteTxnMarkersRequest>("WriteTxnMarkersRequest", 0);
    }
    ...
    proptest! {
        ...
        fn test_java_arbitrary(data: WriteTxnMarkersRequest) {
            crate::test_utils::test_java_arbitrary(&data, "WriteTxnMarkersRequest", 0);
        }
    }
}
