...
impl PartitionData {
    pub fn new(partition_index: i32, replica_epoch: i32, replica_id: i32, replica_directory_id: Uuid, voter_directory_id: Uuid, last_offset_epoch: i32, last_offset: i64, pre_vote: bool) -> Self {
        Self {
            partition_index,
            replica_epoch,
            replica_id,
...
impl Readable for PartitionData {
    fn read(#[allow(unused)] input: &mut impl Read) -> Result<Self> {
        ...
        let _unknown_tagged_fields = read_tagged_fields(input, tagged_fields_callback)?;
        Ok(PartitionData {
            partition_index, replica_epoch, replica_id, replica_directory_id, voter_directory_id, last_offset_epoch, last_offset, pre_vote, _unknown_tagged_fields
        })
    }
}
...
mod tests {
    ...
    fn test_java_default() {
        crate::test_utils::test_java_default::<VoteRequest>("VoteRequest", 2);
    }
    ...
    proptest! {
        ...
        fn test_java_arbitrary(data: VoteRequest) {
            crate::test_utils::test_java_arbitrary(&data, "VoteRequest", 2);
        }
    }
}
