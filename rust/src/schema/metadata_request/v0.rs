...
impl MetadataRequest {
    pub fn new(topics: Vec<MetadataRequestTopic>) -> Self {
        Self {
            topics,
        }
    }
...
impl Readable for MetadataRequest {
    fn read(#[allow(unused)] input: &mut impl Read) -> Result<Self> {
        let topics = read_array::<MetadataRequestTopic>(input, "topics", false)?;
        Ok(MetadataRequest {
            topics
        })
    }
...
/// MetadataRequestTopic, version 0.
#[derive(Serialize, Deserialize, PartialEq, Debug, Clone)]
#[cfg_attr(test, derive(Arbitrary))]
...
mod tests {
    ...
    fn test_java_default() {
        crate::test_utils::test_java_default::<MetadataRequest>("MetadataRequest", 0);
    }
    ...
    proptest! {
        ...
        fn test_java_arbitrary(data: MetadataRequest) {
            crate::test_utils::test_java_arbitrary(&data, "MetadataRequest", 0);
        }
    }
}
