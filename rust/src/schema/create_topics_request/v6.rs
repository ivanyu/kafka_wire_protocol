...
impl Readable for CreatableTopic {
    fn read(#[allow(unused)] input: &mut impl Read) -> Result<Self> {
        ...
        let configs = read_array::<CreatableTopicConfig>(input, "configs", true)?;
        let tagged_fields_callback = |tag: i32, _: &[u8]| {
            match tag {
                _ => Ok(false)
            }
...
/// CreatableReplicaAssignment, version 6.
#[derive(Serialize, Deserialize, PartialEq, Debug, Clone)]
#[cfg_attr(test, derive(Arbitrary))]
...
/// CreatableTopicConfig, version 6.
#[derive(Serialize, Deserialize, PartialEq, Debug, Clone)]
#[cfg_attr(test, derive(Arbitrary))]
...
mod tests {
    ...
    fn test_java_default() {
        crate::test_utils::test_java_default::<CreateTopicsRequest>("CreateTopicsRequest", 6);
    }
    ...
    proptest! {
        ...
        fn test_java_arbitrary(data: CreateTopicsRequest) {
            crate::test_utils::test_java_arbitrary(&data, "CreateTopicsRequest", 6);
        }
    }
}
